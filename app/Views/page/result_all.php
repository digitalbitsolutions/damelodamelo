...
<?= $this->section("css") ?>
    ...
    <script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyAQ9aTmLYmJ84QUaEplrc5K5txuYWN9DpI&libraries=drawing"></script>
<?= $this->endSection() ?>
            ...
            <div class="container-details-header-get">
                <?php 
                    if ($quantity === 0){
                        ...
                        echo "<span>Se encontró ".$quantity." resultados</span>";
                    }
                ?>
                        ...
                        <label for="">
                            ...
                            <div class="select">
                                <select name="ca" id="">
                                    ...
                                    <option value="1" <?= $category_id == 1? "selected" : "" ?>>Arquiler</option>
...
<script>
    ...
    function initMap() {
        map = new google.maps.Map(document.getElementById("map"), {
            ...
            styles: [
                    ...
                    {
                        featureType: "administrative.country", // Países
                        elementType: "labels",
                        stylers: [{ visibility: "on" }]
                    },
                    {
                        featureType: "poi", // Ocultar puntos de interés (negocios, parques, etc.)
                    ...
                    {
                        featureType: "transit", // Ocultar transporte público
                        stylers: [{ visibility: "off" }]
                    },
                    {
    ...
    document.addEventListener("DOMContentLoaded", ()=>{
        const data_for_map = async() =>{
            ...
            await fetch("/api/properties_for_map?"+query_string, {
                method: "GET",
            }).then(res => res.json()).then(data=>{
                if (address_input.value && (!city_input.value && !province_input.value)){
